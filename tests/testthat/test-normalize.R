test_that("basic normalization works", {
    expect_equal(normalize(matrix(c(1,3,3,9,12,0,2,2), nrow=2, ncol=4)),
...
test_that("normalizing zeroed column fails", {
    expect_error(normalize(matrix(c(1,3,3,9,12,0,0,0), nrow=2, ncol=4)), 
...
test_that("normalizing negative values fails", {
    expect_error(normalize(matrix(c(-1,2,2,2), nrow=2, ncol=2)), "values must be non-negative.\n")
})
